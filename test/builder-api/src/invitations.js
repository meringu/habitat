...
describe('Origin Invitations API', function() {
  describe('Invite bobo to xmen', function() {
    ...
    it('returns the invitation', function(done) {
      request.post('/depot/origins/xmen/users/bobo/invitations')
        ...
        .expect(201)
        .end(function(err, res) {
          expect(res.body.account_id).to.equal(global.session_bobo.id);
          expect(res.body.origin_id).to.equal(global.origin_xmen.id);
          expect(res.body.owner_id).to.equal(global.session_logan.id);
          global.invite_bobo_to_xmen = res.body;
          done(err);
        });
    });
    ...
    it('bobo shows up in the origins list of invitations', function(done) {
      request.get('/depot/origins/xmen/invitations')
        ...
        .expect(200)
        .end(function(err, res) {
          expect(res.body.invitations[0].id).to.equal(global.invite_bobo_to_xmen.id);
          expect(res.body.invitations[0].account_id).to.equal(global.session_bobo.id);
          expect(res.body.invitations[0].account_name).to.equal('bobo');
          expect(res.body.invitations[0].origin_id).to.equal(global.origin_xmen.id);
          expect(res.body.invitations[0].origin_name).to.equal('xmen');
          expect(res.body.invitations[0].owner_id).to.equal(global.session_logan.id);
          done(err);
        });
    });
    ...
    it('xmen shows up in bobos list of invitations', function(done) {
      request.get('/user/invitations')
        ...
        .expect(200)
        .end(function(err, res) {
          expect(res.body.invitations[0].origin_invitation_id).to.equal(global.invite_bobo_to_xmen.id);
          expect(res.body.invitations[0].account_id).to.equal(global.session_bobo.id);
          expect(res.body.invitations[0].account_name).to.equal('bobo');
          expect(res.body.invitations[0].origin_id).to.equal(global.origin_xmen.id);
          expect(res.body.invitations[0].origin_name).to.equal('xmen');
  ...
  describe('Bobo accepts the invitation to the xmen', function() {
    it('accepts the invitation', function(done) {
      request.put('/depot/origins/xmen/invitations/' + global.invite_bobo_to_xmen.id)
        .set('Authorization', global.boboBearer)
        .expect(204)
        .end(function(err, res) {
          done(err);
